...
function updateDice() {
  ...
  const multi  = parseFloat((100/safe*0.98).toFixed(2));
...
async function playDice() {
  ...
  const multi  = parseFloat((100/Math.max(1,chance)*0.98).toFixed(2));
  const { gdiceSpent, chipsSpent } = splitBet(bet);

  ["die1","die2"].forEach(id => {
    const d=document.getElementById(id); d.classList.add("roll"); d.textContent="?"; d.style.color="";
  });
  await sleep(600);

  const roll   = Math.floor(Math.random()*100)+1;
  const win    = diceMode==="over" ? roll>target : roll<target;
  const payout = win ? parseFloat((bet*multi).toFixed(4)) : 0;
  const faces  = ["","⚀","⚁","⚂","⚃","⚄","⚅"];
  ["die1","die2"].forEach(id => {
    const d=document.getElementById(id); d.classList.remove("roll");
    d.textContent=faces[Math.floor(Math.random()*6)+1];
    d.style.color=win?"var(--green)":"var(--red)";
  });

  await finishBet("Dice", bet, win?multi:0, payout, win, gdiceSpent, chipsSpent);
  showResult("diceResult", win, win?"🎲":"💸", win?"YOU WIN!":"YOU LOSE",
